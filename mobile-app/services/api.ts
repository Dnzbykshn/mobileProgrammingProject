...
function getExpoHost(): string | null {
  ...
  const hostUri = expoConfig?.hostUri || manifest2?.extra?.expoClient?.hostUri;
...
function getDevBaseUrl(): string {
  ...
  if (expoHost) {
    return `http://${expoHost}:18000/api/v1`;
  }
...
const BASE_URL = __DEV__ ? getDevBaseUrl() : 'https://your-production-api.com/api/v1';
...
async function setAccessToken(token: string): Promise<void> {
  if (await hasSecureStore()) {
    ...
    // Clean up any plaintext fallback token copy.
...
async function getAccessToken(): Promise<string | null> {
  if (await hasSecureStore()) {
    ...
    // One-time migration from AsyncStorage fallback token.
    const fallbackToken = await AsyncStorage.getItem(ACCESS_TOKEN_KEY);
    if (fallbackToken) {
      await SecureStore.setItemAsync(ACCESS_TOKEN_KEY, fallbackToken);
      await AsyncStorage.removeItem(ACCESS_TOKEN_KEY);
    }
...
async function getOrCreateDeviceId(): Promise<string> {
  if (await hasSecureStore()) {
    ...
    const fallbackDeviceId = await AsyncStorage.getItem(DEVICE_ID_KEY);
    const deviceId = fallbackDeviceId || createDeviceId();
