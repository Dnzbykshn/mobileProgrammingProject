...
Requirements:
- App Service running on http://127.0.0.1:18000
...
API_BASE_URL = os.getenv("API_BASE_URL", "http://127.0.0.1:18000/api/v1")
HEALTH_URL = os.getenv("HEALTH_URL", "http://127.0.0.1:18000/health")
OPENAPI_URL = os.getenv("OPENAPI_URL", "http://127.0.0.1:18000/openapi.json")
...
def _seed_prescription_and_pathway_for_user(user_id: str) -> Tuple[str, str]:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                INSERT INTO pathways
                    (id, user_id, prescription_id, title, pathway_type,
    ...
    return prescription_id, pathway_id
...
def main() -> int:
    ...
    try:
        with httpx.Client(timeout=REQUEST_TIMEOUT_SECONDS) as client:
            ...
            prescription_b_id, pathway_b_id = _seed_prescription_and_pathway_for_user(user_b_id)
            ...
            # Cross-user pathway access must be blocked.
            pathway_other = client.get(
                f"{API_BASE_URL}/pathways/{pathway_b_id}",
                headers=_auth_header_with_device(access_a, device_a),
            )
            _expect_status(pathway_other, 404, "cross-user pathway detail")

            # Owner can access own pathway.
            pathway_owner = client.get(
                f"{API_BASE_URL}/pathways/{pathway_b_id}",
                headers=_auth_header_with_device(access_b, device_b),
            )
            _expect_status(pathway_owner, 200, "owner pathway detail")
            ...
            # Logout should revoke the current access token immediately.
