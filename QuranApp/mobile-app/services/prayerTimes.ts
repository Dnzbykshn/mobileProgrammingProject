 ...
 * If all prayers have passed, returns the next day's imsak (if available).
 */
export function getNextPrayer(
    times: PrayerTimesDay,
    now: Date = new Date(),
): { name: PrayerName; label: string; time: string } | null {
    ...
    for (const name of PRAYER_ORDER) {
        ...
        if (h * 60 + m > nowMinutes) {
            return { name, label: PRAYER_LABELS[name], time: timeStr };
        }
    }
 ...
 */
export function countdownUntil(timeStr: string, now: Date = new Date()): string {
