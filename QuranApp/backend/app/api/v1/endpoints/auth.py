...
async def logout(
    ...
    """Best-effort logout with token revocation when Redis is available."""
    if not token:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    if redis_client is not None:
        ...
        exp_ts = int(access_payload.get("exp")) if access_payload and access_payload.get("exp") else None

        if exp_ts:
            ttl = exp_ts - int(datetime.now(timezone.utc).timestamp())
            if ttl > 0:
                blacklist_key = f"auth:blacklist:{token_fingerprint(token)}"
...
async def logout_all(
    ...
    if token and redis_client is not None:
        access_payload = decode_access_token(token)
        exp_ts = int(access_payload.get("exp")) if access_payload and access_payload.get("exp") else None
        if exp_ts:
            ttl = exp_ts - int(datetime.now(timezone.utc).timestamp())
